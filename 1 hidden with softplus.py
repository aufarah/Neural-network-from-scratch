...
class NeuralNetwork:
    ...
    def randominit(self):
        self.weights2   = 2 * np.ones( (self.y.shape[1], self.h1.shape[0]) )      #horizontal per input m, verticaled as y n
        self.bias2      = 1 * np.array([np.ones(self.y.shape[1])]).T

        self.bias1      = 1 * np.array([np.ones(self.h1.shape[0])]).T
        self.weights1   = 2 * np.ones( (self.h1.shape[0], self.input.shape[1]) )
    def feedind(self,x):
    ...
    def backprop(self,x):
        ...
        for e in range(0,m):
            ...
            kali = 2 * (self.y[e] - self.output[e]) * self.drelu(self.output[e])
            kali = np.matrix(kali).T
            ...
            ds_bias1 = self.drelu(self.h1) * np.array( self.weights2.T * kali ) 
            ds_weights1 = np.matrix(self.drelu(self.h1) * self.weights2.T) * kali * np.matrix(self.input[e])      #input sudah gepeng
...
if __name__ == "__main__":
    ...
    yt = binar.test(np.array([3]))
    print(yt)
    yt = binar.test(np.array([4]))
    print(yt)
    yt = binar.test(np.array([5]))
    print(yt)